...
#define R(i) gpr(i)
#define CSR(imm) *read_csr(imm)
#define ECALL(dnpc, no, epc) {dnpc=isa_raise_intr(no, epc);}
#define MRET {restore_interrupt();int mcause=CSR(0x342);switch(mcause){case MSI:s->dnpc=CSR(0x341)+4;break;default:s->dnpc=CSR(0x341);}}
...
void restore_interrupt() { 
  ...
  cpu.csr.mstatus.m.MPIE=1; 
}

vaddr_t *read_csr(word_t imm){
  switch(imm){
    case 0x300:return &(cpu.csr.mstatus.value); break;
...
//extern struct Entry* symbol_tab;
//extern int symbol_tab_size;
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; R(rd) = s->pc + 4; IFDEF(CONFIG_FTRACE, if(rd==1)trace_call(s->pc, s->dnpc)));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = src1 + imm; R(rd) = s->pc + 4; IFDEF(CONFIG_FTRACE, if(rd==1)trace_call(s->pc, s->dnpc);else if(rd==0&&r1==1)trace_ret(s->pc, s->dnpc)));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, imm = imm & 31; R(rd) = (int32_t)src1 >> imm);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = ((int32_t)(Mr(src1 + imm, 1) << 24)) >> 24);
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, ECALL(s->dnpc, MSI, s->pc));
